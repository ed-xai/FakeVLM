...
* For single-image testing, a sample image generated by Gemini is provided at ```imgs/sample_0.png```.

* Added reusable projector-output capture support for single-image inference via `projector_capture.py` and `eval_single_image.py`. Enable it with `--capture_projector_output`; outputs are saved as `.pt` files under a timestamped run folder (format `YYYY_MM_DD_HH_MM_SS`) inside `--projector_save_dir` (default: `./projector_captures`).
